...
# URLs of GitHub
REPO_URL="https://github.com/domparso/hysteria"
...
show_usage_and_exit() {
  ...
  echo -e "\t$0 --check"
  echo
  echo -e "$(tbold)enable hysteria on boot$(treset)"
  echo -e "\t$0 -a"
  echo -e "\t$0 --enable"
  echo -e "\tsystemctl enable  hysteria-server.service"
  echo
  echo -e "$(tbold)Show this help$(treset)"
  echo -e "\t$0 -h"
  echo -e "\t$0 --help"
  echo
  echo -e "$(tbold)start hysteria$(treset)"
  echo -e "\t$0 -S"
  echo -e "\t$0 --start"
  echo
  echo -e "$(tbold)restart hysteria$(treset)"
  echo -e "\t$0 -r"
  echo -e "\t$0 --restart"
  echo
  echo -e "$(tbold)stop hysteria$(treset)"
  echo -e "\t$0 -s"
  echo -e "\t$0 --stop"
  echo
  echo -e "$(tbold)status hysteria$(treset)"
  echo -e "\t$0 -t"
  echo -e "\t$0 --status"
  echo
  echo -e "$(tbold)install hysteria$(treset)"
...
[Service]
Type=simple
ExecStart=$EXECUTABLE_INSTALL_PATH panel --config ${CONFIG_DIR}/${_config_name}.yaml
...
download_hysteria() {
  ...
  # "$REPO_URL/releases/download/app/$_version/hysteria-$OPERATING_SYSTEM-$ARCHITECTURE"
...
perform_install_hysteria_binary() {
  ...
  VERSION=$(curl -s https://api.github.com/repos/domparso/hysteria/releases/latest | grep '"tag_name":' | sed -E 's/.*"tag_name": "([^"]+)".*/\1/')
